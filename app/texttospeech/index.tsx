  ...
  const toggleCameraFacing = () => {
    setFacing((prevFacing) => {
      ...
      console.log('Toggling camera to:', newFacing);
       Speech.speak('Camera flipped to ' + newFacing);
      return newFacing;
    });
  };
  ...
  const captureAndDetectText = async () => {
    try {
      ...
      if (data.responses && data.responses.length > 0) {
        ...
        if (detectedText) {
          console.log('Detected Text:', detectedText);
           ...
           Speech.speak('No text detected. Please try again.');
        }
      } else {
         Speech.speak('No text detected. Please try again.');
      }
      ...
      console.error('Error detecting text:', error);
       Speech.speak('Error detecting text. Please try again.');
    }
  };
  ...
  return (
    <View style={tw`flex-1 bg-black`}>
      ...
      <View style={tw`flex items-center mb-1`}>
        ...
        >
          <TouchableOpacity
            ...
            onPress={() => {
                Speech.speak('Back to home screen');
              router.push('/');
            }}
          >
      ...
      <View style={tw`flex-1 items-center justify-center`}>
        ...
        <View style={tw`mt-4`}>
          <TouchableOpacity
            style={tw`bg-gradient-to-r bg-red-500 rounded-full py-5 px-15 self-center w-3/4`} 
            onPress={captureAndDetectText}
          >
            <Text style={tw`text-white text-xl font-bold text-center`}>Capture</Text>
          </TouchableOpacity>
        </View>
      </View>
